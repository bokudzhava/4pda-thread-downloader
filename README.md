...
*   **Мультипоточность:** Параллельная загрузка нескольких страниц (пачками) для ускорения процесса.
...
2.  Скачайте и установите скрипт: **[Установить скрипт](https://github.com/bokudzhava/4pda-thread-downloader/raw/refs/heads/main/4pda-thread-downloader-1.3.user.js)**
...
3.  Нажмите её, чтобы открыть настройки:
    ...
    *   **Потоков:** Количество одновременных запросов (рекомендуется 3-5).
    *   **Пауза:** Задержка в мс между пачками запросов (рекомендуется 1200+ мс).
...
4.  Нажмите **СКАЧАТЬ**.
5.  Во время загрузки доступны две опции остановки:
    *   **СТОП+СОХР:** Останавливает процесс и сохраняет всё, что успело скачаться.
    *   **ОТМЕНА:** Останавливает процесс без сохранения файла.
6.  Следите за прогрессом на панели. По завершении файл сохранится автоматически.
...
2.  **Лимиты сайта:**
    ...
    *   При использовании **мультипоточности** или опции **"На всех вкладках"** увеличьте паузу до **2000-3000 мс**, так как суммарное количество запросов к серверу возрастает. Частые запросы могут привести к временной блокировке IP (ошибка 403).
